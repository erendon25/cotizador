  ...
  return <div className="quote-page">
    ...
    <div className="quote-layout">
      ...
      <aside className="quote-summary">
        ...
        <dl><div><dt>Costo tecnico</dt><dd>{money(pricing.technicalCost)}</dd></div><div><dt>Precio minimo</dt><dd>{money(pricing.minimumPrice)}</dd></div><div className="suggested"><dt>Tarifa seleccionada</dt><dd>{money(pricing.suggestedPrice)}</dd></div><div className="current"><dt>Precio cotizado</dt><dd>{money(currentPrice)}</dd></div></dl>
...
function ClientStep({ draft, setDraft }: { draft: QuoteDraft; setDraft: React.Dispatch<React.SetStateAction<QuoteDraft>> }) {
  ...
  return <div className="form-content"><div className="form-title"><span><UserRound size={20} /></span><div><h2>Cliente y oportunidad</h2><p>Identifica para quien cotizas y que problema necesita resolver.</p></div></div><div className="field-grid"><label><span>Nombre del contacto</span><input value={draft.client} onChange={(event) => update('client', event.target.value)} placeholder="Ej. Maria Fernandez" /></label><label><span>Empresa o negocio</span><input value={draft.company} onChange={(event) => update('company', event.target.value)} placeholder="Ej. Bodega Central" /></label><label className="full"><span>Nombre del proyecto</span><input value={draft.project} onChange={(event) => update('project', event.target.value)} placeholder="Ej. Sistema de ventas e inventario" /></label><label><span>Presupuesto declarado</span><div className="money-field"><span>S/</span><input type="text" inputMode="numeric" value={draft.budget || ''} placeholder="0" onChange={(event) => update('budget', parseMoneyInput(event.target.value))} /></div><small>Se refleja de inmediato en el resumen; no cambia el valor real del alcance.</small></label><label className="full"><span>Problema principal</span><textarea value={draft.problem} onChange={(event) => update('problem', event.target.value)} placeholder="Describe la operacion actual, los errores y el resultado esperado." rows={4} /></label></div></div>;
}
...
function PriceStep({ draft, setDraft, pricing, currentPrice, discount, phases, modules }: { draft: QuoteDraft; setDraft: React.Dispatch<React.SetStateAction<QuoteDraft>>; pricing: ReturnType<typeof calculatePrice>; currentPrice: number; discount: ReturnType<typeof discountStatus>; phases: ReturnType<typeof proposePhases>; modules: ModuleDefinition[] }) {
  ...
  const belowMinimum = pricing.suggestedPrice > 0 && pricing.suggestedPrice < pricing.minimumPrice;
  return <div className="form-content"><div className="form-title"><span><CircleDollarSign size={20} /></span><div><h2>Precio y estrategia comercial</h2><p>El precio automatico respeta el tarifario; el minimo tecnico se mantiene como control de riesgo.</p></div></div><div className="price-breakdown"><div><span>Total del tarifario</span><strong>{money(pricing.moduleSubtotal)}</strong></div><div><span>Costo interno</span><strong>{money(pricing.technicalCost)}</strong></div><div><span>Minimo con margen</span><strong>{money(pricing.minimumPrice)}</strong></div><div className="highlight"><span>Precio automatico</span><strong>{money(pricing.suggestedPrice)}</strong></div></div>{belowMinimum ? <div className="technical-warning"><strong>Tarifario por debajo del minimo tecnico</strong><p>La diferencia es {money(pricing.minimumPrice - pricing.suggestedPrice)}. El sistema respeta tus precios, pero esta cotizacion no cubre el costo y margen configurados.</p></div> : null}<label className="quoted-field"><span>Precio a cotizar</span><div className="quoted-input-row"><div className="money-field"><span>S/</span><input type="text" inputMode="numeric" value={currentPrice || ''} placeholder="0" onChange={(event) => setDraft((current) => ({ ...current, quotedPrice: parseMoneyInput(event.target.value) }))} /></div><button type="button" className="icon-button" onClick={() => setDraft((current) => ({ ...current, quotedPrice: null }))} title="Usar precio del tarifario actualizado" aria-label="Usar precio del tarifario actualizado"><RotateCcw size={16} /></button></div><small>{draft.quotedPrice === null ? 'Sincronizado con el tarifario actual' : 'Ajuste comercial manual'}</small><small className={discount.label.toLowerCase()}>Descuento {discount.percent.toFixed(1)}% · Estado {discount.label}</small></label>{underBudget ? <div className="budget-warning"><strong>El presupuesto no cubre el alcance completo</strong><p>Faltan {money(pricing.suggestedPrice - draft.budget)}. No se aplico ningun descuento automatico: la opcion responsable es conservar el precio y dividir el alcance.</p></div> : null}<div className="phase-list"><div className="phase-heading"><h3>Propuesta por fases</h3><span>Total preservado: {money(phases.reduce((sum, phase) => sum + phase.amount, 0))}</span></div>{phases.map((phase) => <article key={phase.name}><div><span>{phase.name}</span><strong>{money(phase.amount)}</strong></div><p>{phase.moduleIds.map((id) => modules.find((module) => module.id === id)?.name).filter(Boolean).join(' · ')}</p></article>)}</div></div>;
}
