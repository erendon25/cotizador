...
function PriceStep({ draft, setDraft, pricing, currentPrice, discount, phases, modules }: { draft: QuoteDraft; setDraft: React.Dispatch<React.SetStateAction<QuoteDraft>>; pricing: ReturnType<typeof calculatePrice>; currentPrice: number; discount: ReturnType<typeof discountStatus>; phases: ReturnType<typeof proposePhases>; modules: ModuleDefinition[] }) {
  ...
  return <div className="form-content"><div className="form-title"><span><CircleDollarSign size={20} /></span><div><h2>Precio y estrategia comercial</h2><p>El presupuesto orienta las fases, pero no redefine el valor del trabajo.</p></div></div><div className="price-breakdown"><div><span>Subtotal por modulos</span><strong>{money(pricing.moduleSubtotal)}</strong></div><div><span>Costo interno</span><strong>{money(pricing.technicalCost)}</strong></div><div><span>Minimo con margen</span><strong>{money(pricing.minimumPrice)}</strong></div><div className="highlight"><span>Precio sugerido</span><strong>{money(pricing.suggestedPrice)}</strong></div></div><label className="quoted-field"><span>Precio a cotizar</span><div className="quoted-input-row"><div className="money-field"><span>S/</span><input type="text" inputMode="numeric" value={currentPrice || ''} placeholder="0" onChange={(event) => setDraft((current) => ({ ...current, quotedPrice: parseMoneyInput(event.target.value) }))} /></div><button type="button" className="icon-button" onClick={() => setDraft((current) => ({ ...current, quotedPrice: null }))} title="Usar precio sugerido actualizado" aria-label="Usar precio sugerido actualizado"><RotateCcw size={16} /></button></div><small>{draft.quotedPrice === null ? 'Sincronizado con el tarifario actual' : 'Ajuste comercial manual'}</small><small className={discount.label.toLowerCase()}>Descuento {discount.percent.toFixed(1)}% · Estado {discount.label}</small></label>{underBudget ? <div className="budget-warning"><strong>El presupuesto no cubre el alcance completo</strong><p>Faltan {money(pricing.suggestedPrice - draft.budget)}. No se aplico ningun descuento automatico: la opcion responsable es conservar el precio y dividir el alcance.</p></div> : null}<div className="phase-list"><div className="phase-heading"><h3>Propuesta por fases</h3><span>Total preservado: {money(phases.reduce((sum, phase) => sum + phase.amount, 0))}</span></div>{phases.map((phase) => <article key={phase.name}><div><span>{phase.name}</span><strong>{money(phase.amount)}</strong></div><p>{phase.moduleIds.map((id) => modules.find((module) => module.id === id)?.name).filter(Boolean).join(' · ')}</p></article>)}</div></div>;
}
